    ...
    */
    ...
    public function mobileDetect(){
      ...
      if($is_mobile){
        ...
        if(!$is_tablet){
          $utmsource = '';
          if(strlen($_SERVER['QUERY_STRING']) > 0) $utmsource = '?'.$_SERVER['QUERY_STRING'];
          $currentURL       = $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']; 
          $parsedUrl        = parse_url($currentURL);
          $host             = explode('.', $parsedUrl['path']);
          $segments         = explode('/', $parsedUrl['path']);
          $subdomain        = $host[0];
          $autoshowsegments = array('autoshow', 'kilas', 'mobilbaru', 'mobilkonsep', 'infogiias');
          if($host[1] == "okezonedev"){
            $domain = "http://m2015.okezonedev.com";
            $array_year = array(2015, 2016);
            $subdomain = str_replace($array_year, '', $subdomain);
          }else{
            $domain = "http://m.okezone.com";
          }          
          if(count($segments) <= 1){
            $redirect_url = $domain.'/'.$subdomain.$utmsource;
          }elseif($subdomain == 'foto' || $subdomain == 'video'){
            $redirect_url = $domain.'/'.$subdomain.$utmsource;
          }elseif(count($segments) <= 3 && in_array($segments[1], $autoshowsegments)) {
            $redirect_url = $domain.'/autoshow';

            if ($segments[1] != 'autoshow') {
              $redirect_url .= '/'.$segments[1].$utmsource;
            }
          }elseif(count($segments) <= 3 && strlen($segments[2]) <= 0){
            if($segments[1] == 'telco'){
              $segments[1] = 'telecommunication';
            }elseif($segments[1] == 'ligachampion'){
              $segments[1] = 'ligachampions';
            }elseif($segments[1] == 'movie'){
              $segments[1] = 'movies';
            }elseif($segments[1] == 'music'){
              $segments[1] = 'musik';
            }elseif($segments[1] == 'internasional'){
              $segments[1] = 'international'; 
            }elseif($segments[1] == 'asktheexpert' || $segments[1] == 'index' || $segments[1] == 'tag' || $segments[1] == 'category'){
              $segments[1] = '';
            }
            $redirect_url = $domain.'/'.$subdomain.'/'.$segments[1].$utmsource;
          }else{
            $redirect_url = $domain.'/'.$subdomain.$utmsource;
          }
          
          if($segments[1] == 'read' || $segments[1] == 'view' || $segments[1] == 'play' || $segments[1] == 'topic'){
            $redirect_url = $domain.$_SERVER['REQUEST_URI'];
          }
